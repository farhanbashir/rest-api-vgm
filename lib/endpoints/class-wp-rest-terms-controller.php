...
//		echo "I am here in users"; die;
	 ...
	 *
	 * @param object $product User object.
	 * @param WP_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $product, $request ) {
		$data = array(
			'id'                 => $product->ID,
			'username'           => $product->user_login,
			'name'               => $product->display_name,
			'first_name'         => $product->first_name,
			'last_name'          => $product->last_name,
			'email'              => $product->user_email,
			'url'                => $product->user_url,
			'description'        => $product->description,
			'link'               => get_author_posts_url( $product->ID ),
			'nickname'           => $product->nickname,
			'slug'               => $product->user_nicename,
			'registered_date'    => date( 'c', strtotime( $product->user_registered ) ),
			'roles'              => $product->roles,
			'capabilities'       => $product->allcaps,
			'extra_capabilities' => $product->caps,
		);

		$schema = $this->get_item_schema();

		if ( ! empty( $schema['properties']['avatar_urls'] ) ) {
			$data['avatar_urls'] = rest_get_avatar_urls( $product->user_email );
		}
		 ...
		 * @param object           $product      User object used to create response.
		 * @param WP_REST_Request  $request   Request object.
		 */
		return apply_filters( 'rest_prepare_user', $response, $product, $request );
	}
